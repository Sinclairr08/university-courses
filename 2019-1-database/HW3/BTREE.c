...
/*
1. All nodes has at most m childern
2. every non-leaf node(excpet root) has at least ceil(m/2) children
3. non-leaf node with k children contains k-1 keys
4. all leaves apper in the same level
*/
...
void printTree()
{
	...
	memset(numNodes, 0, level * sizeof(int));
	_mappingNodes(root, nodePtr, numNodes, 0, 0);
