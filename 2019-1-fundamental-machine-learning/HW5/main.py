...
# Convolution Layer
CNN.add_layer('Conv - 1', ConvolutionLayer(in_channels=in_channnel, out_channels=8, kernel_size=3, pad=1, stride = 1))
CNN.add_layer('ReLU - 1', ReLU())
CNN.add_layer('Conv - 2', ConvolutionLayer(in_channels=8, out_channels=8, kernel_size=3, pad=1, stride= 1))
CNN.add_layer('ReLU - 2', ReLU())
CNN.add_layer('Max-Pool - 1', MaxPoolingLayer(kernel_size=2, stride=2))
CNN.add_layer('FC - 1', FCLayer(input_dim=1568, output_dim=500))
CNN.add_layer('FC - 2', FCLayer(input_dim=500, output_dim=5))
