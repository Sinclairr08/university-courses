...
def convolution2d(x, kernel, stride):
    ...
    for ind_x in range(conv_height):
        for ind_y in range(conv_width):
            conv_out[ind_x][ind_y] = np.sum(np.multiply(x[ind_x * stride : ind_x * stride + kernel_size,
                                                 ind_y * stride : ind_y * stride + kernel_size], kernel))
    # =========================================================================
    return conv_out
        ...
        for batch in range(batch_size):
            for out in range(out_channel):
                for inp in range(self.W.shape[1]):
                    conv[batch,out,:,:] += convolution2d(x[batch,inp,:,:], self.W[out,inp,:,:], self.stride)
                    if bias is not None:
        ...
        for batch in range(batch_size):
            for out in range(out_channel):
                for inp in range(in_channel):
                    self.dW[out, inp] += convolution2d(self.x[batch, inp], d_prev[batch, out], self.stride)
                    if self.pad:
                        dx[batch, inp] += convolution2d(d_prev_expand[batch, out], W_rotate[out, inp],  self.stride)[self.pad:-self.pad, self.pad:-self.pad]
                    else:
                        dx[batch, inp] += convolution2d(d_prev_expand[batch, out], W_rotate[out, inp], self.stride)
            ...
            for chan in range(channel):
                for ind_x in range(pool_height):
                    for ind_y in range(pool_width):
                        offset = np.unravel_index(np.argmax(x[batch, chan, ind_x * self.stride : ind_x * self.stride + self.kernel_size,
                                                            ind_y * self.stride : ind_y * self.stride + self.kernel_size]), (self.kernel_size, self.kernel_size))
            ...
            for chan in range(channel):
                for h in range(height):
                    for w in range(width):
                        d_prev_expand[bat, chan, h * self.stride : h * self.stride + self.kernel_size, w * self.stride : w * self.stride + self.kernel_size] = d_prev[bat, chan, h, w]
